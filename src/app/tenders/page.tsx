...
export default async function TendersPage({
  ...
  const [tenders, sectors] = await Promise.all([
    getTenders(sectorSlug),
    getAllSectors(),
  ])

  const activeSector = sectorSlug ? sectors.find((s) => s.slug === sectorSlug) : null
  const totalValue   = tenders.reduce((sum, t) => sum + (t.valueCrore ?? 0), 0)
  const avgDelivery  = tenders.length
    ? Math.round(tenders.reduce((s, t) => s + (t.completionMonths ?? 0), 0) / tenders.length)
    : 0
  ...
  return (
    <main className="max-w-6xl mx-auto px-4 py-12 space-y-8">
      ...
      <div className="grid grid-cols-3 gap-px rounded-xl overflow-hidden bg-border border">
        {[
          { label: "Tenders shown", value: tenders.length.toString(), color: "text-blue-600" },
          { label: "Total value",   value: fmt(totalValue),            color: "text-emerald-600" },
          { label: "Avg. delivery", value: `${avgDelivery} mo`,        color: "text-sky-600" },
            ...
            return (
              <div key={tender.id} className="px-5 py-5 hover:bg-muted/20 transition-colors">
                ...
                <div className="flex items-center gap-2 mt-3 flex-wrap text-xs">
                  <span className="font-semibold" style={{ color: tender.sector.color ?? undefined }}>
                    {tender.sector.name}
                  </span>
                  ...
                  {schemeName && (
                    <>
                      <span className="text-muted-foreground">·</span>
                      <span className="text-blue-600 font-medium">{schemeName}</span>
                    </>
                  )}
                  {tender.awardedAt && (
