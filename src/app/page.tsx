...
import { SECTORS, REFORMS, COMPANIES, TENDERS, STATS } from "@/lib/seed-data"
...
// Build a unified, date-sorted activity feed
type FeedItem =
  | { kind: "tender"; date: Date; data: (typeof TENDERS)[number] }
  | { kind: "reform"; date: Date; data: (typeof REFORMS)[number] }

const FEED: FeedItem[] = [
  ...TENDERS.map((t) => ({ kind: "tender" as const, date: t.awardedAt, data: t })),
  ...REFORMS.filter((r) => r.notifiedAt).map((r) => ({
...
// Pick the 3 highlight signals
const topTender   = TENDERS[0]
const topReform   = REFORMS.find((r) => r.status === "NOTIFIED" || r.status === "PROPOSED")!
const topSector   = [...SECTORS].sort((a, b) => b.govtOutlayCrore - a.govtOutlayCrore)[0]
    ...
    <main className="max-w-6xl mx-auto px-4 py-10 space-y-12">
      ...
      <section className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 pb-6 border-b">
        ...
        <div className="flex gap-6 shrink-0">
          {[
            { value: fmt(STATS.capexCrore),          label: "capex tracked",   color: "text-blue-600" },
            { value: fmt(STATS.fdiCommittedCrore),    label: "FDI committed",   color: "text-emerald-600" },
            { value: `+${STATS.avgOrderBookGrowthPct}%`, label: "avg order growth", color: "text-sky-600" },
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          ...
          <Link href={`/companies/${topTender.company.slug}`}
            ...
            <div className="flex items-end justify-between mt-3">
              <div>
                <p className="font-display text-2xl text-emerald-700">{fmt(topTender.valueCrore)}</p>
          ...
          <Link href={`/reforms/${topReform.slug}`}
            ...
            <span className={`text-[10px] font-semibold uppercase tracking-widest rounded px-2 py-0.5 inline-block mb-3 border ${STATUS_STYLES[topReform.status]}`}>
            ...
            <div className="flex items-end justify-between mt-3">
              ...
              <span className="text-xs font-medium border rounded px-2 py-0.5"
                style={{ color: topReform.sector.color }}>
          ...
          <Link href={`/companies?sectorSlug=${topSector.slug}`}
            ...
            <div className="absolute top-0 right-0 w-24 h-24 rounded-full blur-2xl opacity-10 pointer-events-none"
              style={{ backgroundColor: topSector.color }} />
            <span className="text-[10px] font-semibold uppercase tracking-widest rounded px-2 py-0.5 inline-block mb-3 border"
              style={{ color: topSector.color, borderColor: `${topSector.color}50`, backgroundColor: `${topSector.color}12` }}>
              Sector Spotlight
            </span>
            ...
            <div className="flex items-end justify-between mt-3">
              <div>
                <p className="font-display text-2xl" style={{ color: topSector.color }}>{fmt(topSector.govtOutlayCrore)}</p>
          ...
          {FEED.slice(0, 8).map((item, i) => {
              ...
              return (
                <div key={`t-${i}`} className="flex items-center gap-4 px-4 py-3 hover:bg-muted/20 transition-colors">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <span className="text-xs font-bold tabular-nums text-emerald-700">{fmt(t.valueCrore)}</span>
            ...
            return (
              <Link key={`r-${i}`} href={`/reforms/${r.slug}`}
                ...
                <span className={`text-[10px] font-semibold uppercase tracking-wide rounded px-1.5 py-0.5 shrink-0 border ${STATUS_STYLES[r.status]}`}>
            ...
            return (
              ...
              >
                ...
                <div className="grid grid-cols-3 gap-1.5 mb-3 text-center">
                  ...
                  <div className="bg-muted/50 rounded-lg py-1.5 px-1">
                    <p className={`text-xs font-bold tabular-nums ${co.roce > 20 ? "text-emerald-700" : ""}`}>
                      {co.roce}%
                    </p>
