...
import { getSchemeBySlug, getSchemes } from "@/lib/data/schemes"
...
export default async function SchemeDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const disbursedPct = scheme.budgetOutlayCrore
    ? Math.round(((scheme.disbursedCrore ?? 0) / scheme.budgetOutlayCrore) * 100)
    : 0
  const multiplier = scheme.budgetOutlayCrore
    ? ((scheme.totalInvestmentCrore ?? 0) / scheme.budgetOutlayCrore).toFixed(1)
    : "—"

  const beneficiaryCompanies = scheme.companies.map((cs) => cs.company)
  const linkedReforms        = scheme.reforms
  const sectorNames          = scheme.sectors.map((ss) => ss.sector.name)

  // Related schemes share at least one sector
  const allSchemes    = await getSchemes()
  const relatedSchemes = allSchemes.filter(
    (s) => s.id !== scheme.id && s.sectors.some((ss) => sectorNames.includes(ss.sector.name))
  )
  ...
  return (
    <main className="max-w-4xl mx-auto px-4 py-12 space-y-10">
      ...
      <nav className="flex items-center gap-2 text-xs text-muted-foreground">
        ...
        <span>›</span>
        <span className="text-foreground">{scheme.shortName ?? scheme.name}</span>
      </nav>

      {/* Hero */}
      <div className="relative border rounded-2xl overflow-hidden bg-card">
        <div className="h-1.5 w-full" style={{ backgroundColor: scheme.color ?? undefined }} />
        <div className="p-8">
          <div className="absolute top-8 right-8 w-64 h-64 rounded-full blur-3xl opacity-10 pointer-events-none"
            style={{ backgroundColor: scheme.color ?? undefined }} />
          <div className="relative">
            <div className="flex flex-wrap items-center gap-2 mb-3">
              ...
              {scheme.sectors.map(({ sector }) => (
                <span key={sector.id} className="text-xs font-medium px-2.5 py-1 rounded-full border"
            ...
            {scheme.highlight && (
              <p className="mt-3 text-sm font-medium border-l-2 pl-3 italic text-muted-foreground"
                style={{ borderColor: scheme.color ?? undefined }}>
                {scheme.highlight}
              </p>
            )}
          </div>
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-px rounded-xl overflow-hidden bg-border border">
        {[
          { label: "Budget Outlay",  value: fmt(scheme.budgetOutlayCrore ?? 0),    sub: "govt commitment",       color: "text-violet-600" },
          { label: "Disbursed",      value: fmt(scheme.disbursedCrore ?? 0),        sub: `${disbursedPct}% released`, color: "text-blue-600" },
          { label: "Investment In",  value: fmt(scheme.totalInvestmentCrore ?? 0),  sub: `${multiplier}× multiplier`, color: "text-emerald-600" },
      ...
      <section className="border rounded-xl p-6 bg-card">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-xs font-bold tabular-nums" style={{ color: scheme.color ?? undefined }}>{disbursedPct}%</span>
        </div>
        <div className="h-3 bg-muted rounded-full overflow-hidden mb-3">
          <div className="h-full rounded-full transition-all" style={{ width: `${disbursedPct}%`, backgroundColor: scheme.color ?? undefined }} />
        </div>
        <div className="flex justify-between text-xs text-muted-foreground">
          <span>{fmt(scheme.disbursedCrore ?? 0)} disbursed</span>
          <span>{fmt((scheme.budgetOutlayCrore ?? 0) - (scheme.disbursedCrore ?? 0))} remaining</span>
        </div>
        <div className="mt-4 grid grid-cols-2 gap-4 pt-4 border-t">
          <div>
            <p className="text-[10px] text-muted-foreground uppercase tracking-wide mb-1">Approved Projects</p>
            <p className="text-lg font-bold tabular-nums">{(scheme.approvedProjects ?? 0).toLocaleString("en-IN")}</p>
          </div>
          <div>
      ...
      {/* Beneficiaries */}
      <section>
        <h2 className="text-xs font-medium text-muted-foreground uppercase tracking-widest mb-4">Key Beneficiaries</h2>
        ...
        {scheme.keyBeneficiaries.length > 0 && (
          <div>
          ...
          <div className="divide-y border rounded-xl overflow-hidden bg-card">
            {linkedReforms.map((reform) => (
              <Link key={reform.id} href={`/reforms/${reform.slug}`}
                ...
                <span className={`text-xs font-semibold px-2.5 py-1 rounded-full border shrink-0 ${statusColors[reform.status as keyof typeof statusColors] ?? statusColors.PROPOSED}`}>
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            {relatedSchemes.map((s) => (
              <Link key={s.id} href={`/schemes/${s.slug}`}
                ...
                <div className="w-1 self-stretch rounded-full shrink-0" style={{ backgroundColor: s.color ?? undefined }} />
                ...
                <div className="text-right shrink-0">
                  <p className="text-sm font-bold tabular-nums text-violet-700">{fmt(s.budgetOutlayCrore ?? 0)}</p>
