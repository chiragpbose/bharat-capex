...
import { getSchemes } from "@/lib/data/schemes"
...
export default async function SchemesPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const [filtered, allSchemes] = await Promise.all([
    getSchemes(sector),
    getSchemes(),
  ])

  const totalOutlay     = allSchemes.reduce((s, sc) => s + (sc.budgetOutlayCrore ?? 0), 0)
  const totalDisbursed  = allSchemes.reduce((s, sc) => s + (sc.disbursedCrore ?? 0), 0)
  const totalInvestment = allSchemes.reduce((s, sc) => s + (sc.totalInvestmentCrore ?? 0), 0)
  const totalJobs       = allSchemes.reduce((s, sc) => s + (sc.jobsTargeted ?? 0), 0)

  const uniqueSectors = Array.from(
    new Set(allSchemes.flatMap((s) => s.sectors.map((ss) => ss.sector.name)))
  )
    ...
    <main className="max-w-6xl mx-auto px-4 py-12 space-y-10">
      ...
      <div className="border rounded-xl p-5 bg-card space-y-2">
        ...
        <div className="h-2.5 bg-muted rounded-full overflow-hidden">
          <div
            ...
            style={{ width: `${totalOutlay ? Math.round((totalDisbursed / totalOutlay) * 100) : 0}%` }}
          />
        </div>
        <p className="text-xs text-muted-foreground">
          {totalOutlay ? Math.round((totalDisbursed / totalOutlay) * 100) : 0}% disbursed across {allSchemes.length} active schemes
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {uniqueSectors.map((name) => {
          const sec = allSchemes.flatMap((s) => s.sectors).find((ss) => ss.sector.name === name)?.sector
          ...
          return (
            <Link
              ...
              style={isActive && sec ? { color: sec.color ?? undefined, borderColor: `${sec.color}60`, backgroundColor: `${sec.color}12` } : {}}
            >
              {name}
            </Link>
          )
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {filtered.map((scheme) => {
          ...
          return (
            ...
            >
              <div className="h-1 w-full" style={{ backgroundColor: scheme.color ?? undefined }} />
              ...
              <div className="p-5 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <p className="font-display text-xl tabular-nums text-violet-700">{fmt(scheme.budgetOutlayCrore ?? 0)}</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wide mb-0.5">Investment attracted</p>
                    <p className="font-display text-xl tabular-nums text-blue-700">{fmt(scheme.totalInvestmentCrore ?? 0)}</p>
                ...
                <div>
                  <div className="flex justify-between text-[10px] text-muted-foreground mb-1.5">
                    ...
                    <span className="tabular-nums font-medium text-foreground">
                      {fmt(scheme.disbursedCrore ?? 0)} <span className="text-muted-foreground font-normal">({disbursedPct}%)</span>
                    </span>
                  </div>
                  <div className="h-1.5 bg-muted rounded-full overflow-hidden">
                    <div
                      className="h-full rounded-full transition-all"
                      style={{ width: `${disbursedPct}%`, backgroundColor: scheme.color ?? undefined }}
                    />
                  </div>
                </div>
                ...
                <div className="flex items-end justify-between gap-3 pt-1">
                  <div className="flex flex-wrap gap-1">
                    {scheme.sectors.map(({ sector: sec }) => (
                      <span
                        key={sec.id}
                        className="text-[10px] font-medium px-2 py-0.5 rounded-full border"
                        style={{ color: sec.color ?? undefined, borderColor: `${sec.color}50`, backgroundColor: `${sec.color}0d` }}
                      >
                        {sec.name}
                      </span>
                    ))}
